...
export function initDB() {
  ...
  try {
    const ftsCount = (db.prepare('SELECT COUNT(*) AS c FROM verses_fts').get() as CountRow | undefined)?.c || 0
    const versesCount = (db.prepare('SELECT COUNT(*) AS c FROM verses').get() as CountRow | undefined)?.c || 0
