...
import { db } from './db'
import { VerseRecord } from './osisLoader'
...
function parseOldBinaryEwb(buffer: Buffer): VerseRecord[] {
  const magic = buffer.slice(0, 22).toString('utf8')
  if (magic !== OLD_EWB_MAGIC) return []
  ...
  const books: Array<{ name: string; chapterCounts: number[]; offset: number; length: number }> = []

  for (let i = 0; i < 66; i += 1) {
    const recordOffset = bookTableOffset + i * bookRecordLength
    if (recordOffset + bookRecordLength > buffer.length) break
...
function parseSqliteEwb(filePath: string): VerseRecord[] {
  const external = new Database(filePath, { readonly: true, fileMustExist: true })
  try {
    const tables = external.prepare("SELECT name FROM sqlite_master WHERE type='table'").all() as Array<{ name: string }>
    for (const table of tables) {
      const columns = external.prepare(`PRAGMA table_info(${JSON.stringify(table.name)})`).all() as Array<{ name: string }>
      ...
      if (!bookCol || !chapterCol || !verseCol || !textCol) continue

      const rows = external.prepare(
        `SELECT ${JSON.stringify(bookCol)} AS book, ${JSON.stringify(chapterCol)} AS chapter, ${JSON.stringify(verseCol)} AS verse, ${JSON.stringify(textCol)} AS text FROM ${JSON.stringify(table.name)}`
      ).all() as Array<{ book: string | number; chapter: number; verse: number; text: string }>
...
export async function importEasyWorshipBibleFromFile(
  ...
  const existing = db.prepare('SELECT id FROM bibles WHERE translation = ?').get(translationCode) as { id: number } | undefined
  if (existing?.id) {
  ...
  const insertTx = db.transaction(() => {
    ...
    verses.forEach((verse, index) => {
      ...
      if (index % 1000 === 0) onProgress?.(20 + Math.round((index / verses.length) * 75))
    })
    return bibleId
  })
