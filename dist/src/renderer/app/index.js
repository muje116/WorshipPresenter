...
const stripChordMarkup = (text) => (0, RichText_1.richTextToPlainText)(text).replace(/\[([^\]]+)\]/g, '').replace(/\s+/g, ' ').trim();
...
const AppInner = () => {
    ...
    const sendLiveState = (slideTitle) => {
        const targetOutputIds = activeOutputWindows.length
            ...
            : OUTPUT_IDS;
        const slideHtml = (0, RichText_1.hasRichTextMarkup)(slideTitle) ? (0, RichText_1.getRichTextHtml)(slideTitle) : undefined;
        const state = {
            slideTitle: (0, RichText_1.richTextToPlainText)(slideTitle),
            ...(slideHtml ? { slideHtml } : {}),
            mediaPath: '',
            mediaType: undefined,
            theme,
        };
        targetOutputIds.forEach((id) => window?.worship?.outputs?.setState?.(id, state));
    };
    ...
    const paletteCommands = [
        ...
        { id: 'add-song', label: 'Add New Song', icon: '➕', category: 'Library', action: async () => { const id = await addSong(); if (id)
                setSelectedSongId(id); setWorkspace('editor'); } },
